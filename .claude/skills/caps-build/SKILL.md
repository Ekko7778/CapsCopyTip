...
读取 `CursorTip.ahk` 中的当前版本号（`global VERSION := "x.y.z"`），用于输出文件名。

**不要自动递增版本号**。版本号由 git 提交时管理，编译只使用当前值。

仅当用户**明确要求**更新版本号（如"更新版本号"、"递增版本"）时，才按二十进制递增 PATCH 位：
...
3. 编译输出为 `CursorTip_v2.0.3.exe`
4. 报告结果

**用户说："编译，更新版本号"**
1. 读取当前版本号 v2.0.3 → 递增为 v2.0.4
2. 更新 `CursorTip.ahk` 中的 `VERSION`
3. 检查资源文件是否都已 FileInstall 嵌入
4. 编译输出为 `CursorTip_v2.0.4.exe`
5. 报告结果
